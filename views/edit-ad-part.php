        ...
        <form id="email-form" name="email-form" data-name="Email Form" method="get" class="form-6"
            ...
            <div class="div-block-743" id="error" style="display: none;">
                <div class="div-block-744">
                    <div id="errorText">Došlo je do greške prilikom ažuriranja oglasa. Molimo pokušajte kasnije</div>
                </div>
            </div>
            ...
            <div class="div-block-745">
                ...
                <div class="div-block-747 twoinrow">
                    <select id="brandSelect" name="Brand select" data-name="Brand" class="select-field w-select">
                        <option value="<?php echo $adData['brand'] ?>">
                ...
                <div class="div-block-747 twoinrow">
                    <label id="newState" data-w-id="bc18a5ea-5841-bf10-8f85-d037b0bf372d"
                        ...
                        <input type="radio" id="newState" name="deviceState" value="newState" data-name="newState"
                            ...
                            <?php if($adData['state']==1) echo 'checked'; ?> />
                        <span class="radio-button-label w-form-label" for="newState-2">Nov uređaj</span>
                    </label>
                    <label id="oldState" data-w-id="3725dcd0-e3f5-4a0f-e904-a5b904091c7d"
                        ...
                        <input type="radio" id="oldState" name="deviceState" value="oldState" data-name="oldState"
                            class="w-form-formradioinput radio-button w-radio-input" onclick="checkUserState()" 
                            <?php if($adData['state']==0) echo 'checked'; ?> />
            ...
            <div class="div-block-745" id="deviceStateRate">
                ...
                <div class="div-block-747 slider-container">
                    <input type="range" min="0" max="10" step="0.1" value="<?php echo $adData['stateRange'] ?>" id="myRange" oninput="updateValue()">
                    <div
                    ...
                    <div id="selected-value" style="font-weight: bold">Stanje uređaja: <?php echo $adData['stateRange'] ?> / 10</div>
                </div>
            </div>
                        ...
                        if (is_dir($folderPath)) {
                            ...
                            foreach ($files as $file) {
                                ...
                                if (strtolower($fileNameWithoutExtension) === 'mainimage') {
                                    $mainImage = "../uploads/" . $adData['images'] . '/' . $file;
                                    break;
                                }
                            }
                            ?>
            ...
            <div class="div-block-745">
                ...
                <div class="div-block-747">
                    <?php
                        $accessories = explode(',', $adData['accessories']);
                        array_pop($accessories);
                    ?>
                    <div class="div-block-751" id="selectedAccessories" style="display:<?php echo !empty($accessories) ? 'block' : 'none'?>">
                        <?php
                            if(!empty($accessories)) {
                                ?>
                                    <script>
                                        let selectedAccessoriesTemp = [];
                                    </script>
                                <?php
                                foreach ($accessories as $access) {
                                    ?>
                                        <script>
                                            selectedAccessoriesTemp.push('<?php echo $access ?>');
                                        </script>
                                    <?php
                                }
                            }
                    ...
                    <div class="div-block-751" id="selectedDamages">
                        <?php
                            if(!empty($damages)) {
                                ?>
                                    <script>
                                        let selectedDamagesTemp = [];
                                    </script>
                                <?php
                                foreach ($damages as $dam) {
                                    ?>
                                        <script>
                                            selectedDamagesTemp.push('<?php echo $dam ?>');
                                        </script>
                                    <?php
                                }
                            }
                ...
                <div class="div-block-747">
                    ...
                    <div class="main-div">
                        <div id="editor" contenteditable="true" oninput="updateCharacterCount(0)"
                            onkeydown="onKeyDown(event)" maxlength="2000">
                            <?php echo $adData['description'] ?>
                        </div>
                        <div id="character-count">
                            <p style="margin: 0;"><?php echo strlen($adData['description']) ?> / 2000</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="div-block-747 twoinrow">
                    <div class="div-block-755">
                        <input type="number" class="text-field-6 w-input" maxlength="256" name="price" data-name="price"
                            placeholder="" id="price" value="<?php echo $adData['price'] != null ? $adData['price'] : '' ?>" />
                        ...
                        <input type="checkbox" id="deal" name="deal" data-name="deal"
                            class="w-checkbox-input checkbox-2" onclick="dealOrPrice()" <?php if($adData['price'] == null) echo 'checked' ?>/>
            ...
            <div class="div-block-758" style="margin-bottom: 20px;">
                <input type="button" value="Ažuriraj oglas" data-value="Ažuriraj oglas" update="<?php echo $adData['ad_id'] ?>"
